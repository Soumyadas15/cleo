...
export async function POST(req: Request) {
  try {
    ...
    //@ts-ignore
    const formattedAudits = project.audits ? project.audits.map(audit => ({
    ...
    //@ts-ignore
    const formattedResources = project.resources ? project.resources.map(resource => ({
      Name: resource.name,
    ...
    //@ts-ignore
    const formattedFedbacks = project.feedbacks ? project.feedbacks.map(feedback => ({
    ...
    //@ts-ignore
    const formattedUpdates = project.updates ? project.updates.map(update => ({
      Date: format(new Date(update.date), 'MM/dd/yyyy'),
      Body: update.body,
    })) : [];
    //@ts-ignore
    const formattedSprints = project.sprints ? project.sprints.map(sprint => ({
    ...
    //@ts-ignore
    const formattedMoms = project.moms ? project.moms.map(mom => ({
      Date: format(new Date(mom.date), 'MM/dd/yyyy'),
      Body: mom.comments,
    })) : [];
    //@ts-ignore
    const stakeHolders = project.stakeholders ? project.stakeholders.map(stakeholder => ({
      Title: stakeholder.title,
      Name: stakeholder.name,
      Contact: stakeholder.contact
    })) : [];
    //@ts-ignore
    const formattedMilestones = project.milestones ? project.milestones.map(milestone => ({
    ...
    //@ts-ignore
    const formattedRiskProfile = project.risks ? project.risks.map(risk => ({
      Type: risk.type,
    ...
    //@ts-ignore
    const formattedVersion = project.versions ? project.versions.map(version => ({
